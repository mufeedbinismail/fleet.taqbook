    ...
    const instance = {
        ...
        set(value) {
            const dates = (Array.isArray(value) ? value : [value])
                .map((one) => bound(one, format, locale))
                .filter((one) => one !== null);
...
export function mount(el, raw = {}) {
    ...
    const earliest = bound(config.min, format, locale);
    const latest = bound(config.max, format, locale);
    ...
    const instance = {
        ...
        limits({ min = null, max = null } = {}) {
            holdingTheValue(() =>
                picker.update({
                    selectedDates: false,
                    minDate: bound(min, format, locale) ?? false,
                    maxDate: bound(max, format, locale) ?? false,
                }),
            );
        },
...
function read(el, format, locale) {
    const found = bound(el.value.trim(), format, locale);

    return found === null ? [] : [found];
}

/**
 * A date stated as an instant or as text in either spelling; an instant is taken as itself.
 */
function bound(value, format, locale) {
    if (value instanceof Date) return Number.isNaN(value.getTime()) ? null : value;

    if (!value) return null;

    for (const reading of [format, ...MACHINE_FORMATS]) {
        const found = parseDate(String(value), reading, locale);
...
function edge(value, format, locale) {
    const parsed = bound(value, format, locale);
