...
const MACHINE_DATE_TIME_FORMAT = 'Y-m-d H:i:s';
const MACHINE_TIME_FORMAT = 'H:i:s';

function machineFormat(format) {
    if (!hasDate(format)) return MACHINE_TIME_FORMAT;

    return hasTime(format) ? MACHINE_DATE_TIME_FORMAT : MACHINE_DATE_FORMAT;
}

/**
 * The given spelling first, so a text both spellings accept is read the way the given one means it.
 */
export function readDate(value, format, locale = DEFAULT_LOCALE) {
    if (value instanceof Date) return Number.isNaN(value.getTime()) ? null : value;

    if (!value) return null;

    for (const reading of [format, machineFormat(format)]) {
