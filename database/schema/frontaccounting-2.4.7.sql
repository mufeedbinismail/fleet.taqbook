-- FrontAccounting 2.4.7, the schema this application was forked from.
--
-- Recovered from `sql/en_US-new.sql` at commit 4cdb8c87, changed in four ways.
--
-- The `0_` table prefix is stripped and the `DROP TABLE IF EXISTS` statements are removed, so
-- that installing this can only ever add. `SET NAMES latin1` is removed because the bytes are
-- UTF-8 in spite of that declaration, and honouring it stores the one non-ASCII character here
-- as mojibake.
--
-- Every table is pinned to `DEFAULT CHARSET=latin1`, which the original left to whatever the
-- database defaulted to. Widening a column to utf8mb4 later grows its type to hold the same
-- number of characters in wider ones — tinytext becomes text — so a base installed under a
-- charset that is already wide yields narrower columns than one installed under a narrow one.
-- Unpinned, the schema depends on how somebody happened to create the database.
