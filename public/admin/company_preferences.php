...
$GLOBALS['page_security'] = Permission::CONFIGURE_COMPANY;
require __DIR__ . "/../includes/session.inc";

page(__($GLOBALS['help_context'] = "Company Setup"));
...
if (isset($_POST['update']) && $_POST['update'] != "")
{
	...
	{
		display_error(__("Login timeout must be positive number not less than 10."));
	...
	{
		$input_error = 1;
		display_error(__("The company name must be entered."));
		set_focus('coy_name');
	}
	if (!check_num('tax_prd', 1))
	{
		display_error(__("Tax Periods must be positive number."));
		set_focus('tax_prd');
		$input_error = 1;
	}
	if (!check_num('tax_last', 1))
	{
		display_error(__("Tax Last Periods must be positive number."));
		set_focus('tax_last');
		$input_error = 1;
	}
	if (!check_num('round_to', 1))
	{
		display_error(__("Round Calculated field must be a positive number."));
		set_focus('round_to');
		$input_error = 1;
	}
	if (!check_num('max_days_in_docs', 1))
	{
		display_error(__("Max day range in Documents must be a positive number."));
		set_focus('max_days_in_docs');
		$input_error = 1;
	}
	if ($_POST['add_pct'] != "" && !is_numeric($_POST['add_pct']))
	{
		display_error(__("Add Price from Std Cost field must be number."));
		set_focus('add_pct');
		$input_error = 1;
	}	
    ...
    elseif ($_FILES['pic']['error'] > 0) {
			display_error(__('Error uploading logo file.'));
			$input_error = 1;
    }
		...
		{
			display_error(__('Only jpg and png files are supported - a file extension of .jpg or .png is expected'));
			$input_error = 1;
		}
		elseif ( $_FILES['pic']['size'] > ($SysPrefs->max_image_size * 1024))
		{ //File Size Check
			display_error(__('The file size is over the maximum allowed. The maximum size allowed in KB is') . ' ' . $SysPrefs->max_image_size);
			$input_error = 1;
		}
		elseif ( $_FILES['pic']['type'] == "text/plain" )
		{  //File type Check
			display_error( __('Only graphics files can be uploaded'));
			$input_error = 1;
		}
	...
	if ($input_error != 1)
	{
		...
		display_notification_centered(__("Company setup has been updated."));
		set_focus('coy_name');
...
text_row_ex(__("Name (to appear on reports):"), 'coy_name', 50, 50);
textarea_row(__("Address:"), 'postal_address', $_POST['postal_address'], 34, 5);
text_row_ex(__("Domicile:"), 'domicile', 25, 55);

text_row_ex(__("Phone Number:"), 'phone', 25, 55);
text_row_ex(__("Fax Number:"), 'fax', 25);
email_row_ex(__("Email Address:"), 'email', 50, 55);

email_row_ex(__("BCC Address for all outgoing mails:"), 'bcc_email', 50, 55);

text_row_ex(__("Official Company Number:"), 'coy_no', 25);
text_row_ex(__("GSTNo:"), 'gst_no', 25);
currencies_list_row(__("Home Currency:"), 'curr_default', $_POST['curr_default']);

label_row(__("Company Logo:"), $_POST['coy_logo']);
file_row(__("New Company Logo (.jpg)") . ":", 'pic', 'pic');
check_row(__("Delete Company Logo:"), 'del_coy_logo', $_POST['del_coy_logo']);

check_row(__("Time Zone on Reports"), 'time_zone', $_POST['time_zone']);
check_row(__("Company Logo on Reports"), 'company_logo_report', $_POST['company_logo_report']);
check_row(__("Use Barcodes on Stocks"), 'barcodes_on_stock', $_POST['barcodes_on_stock']);
check_row(__("Auto Increase of Document References"), 'ref_no_auto_increase', $_POST['ref_no_auto_increase']);
check_row(__("Use Dimensions on Recurrent Invoices"), 'dim_on_recurrent_invoice', $_POST['dim_on_recurrent_invoice']);
check_row(__("Use Long Descriptions on Invoices"), 'long_description_invoice', $_POST['long_description_invoice']);
check_row(__("Company Logo on Views"), 'company_logo_on_views', $_POST['company_logo_on_views']);
label_row(__("Database Scheme Version"), $_POST['version_id']);

table_section(2);

table_section_title(__("General Ledger Settings"));
fiscalyears_list_row(__("Fiscal Year:"), 'f_year', $_POST['f_year']);
text_row_ex(__("Tax Periods:"), 'tax_prd', 10, 10, '', null, null, __('Months.'));
text_row_ex(__("Tax Last Period:"), 'tax_last', 10, 10, '', null, null, __('Months back.'));
check_row(__("Put alternative Tax Include on Docs"), 'alternative_tax_include_on_docs', null);
check_row(__("Suppress Tax Rates on Docs"), 'suppress_tax_rates', null);
check_row(__("Automatic Revaluation Currency Accounts"), 'auto_curr_reval', $_POST['auto_curr_reval']);

table_section_title(__("Sales Pricing"));
sales_types_list_row(__("Base for auto price calculations:"), 'base_sales', $_POST['base_sales'], false,
    __('No base price list') );

text_row_ex(__("Add Price from Std Cost:"), 'add_pct', 10, 10, '', null, null, "%");
$curr = get_currency($_POST['curr_default']);
text_row_ex(__("Round calculated prices to nearest:"), 'round_to', 10, 10, '', null, null, $curr['hundreds_name']);
label_row("", "&nbsp;");


table_section_title(__("Optional Modules"));
check_row(__("Manufacturing"), 'use_manufacturing', null);
check_row(__("Fixed Assets"), 'use_fixed_assets', null);
number_list_row(__("Use Dimensions:"), 'use_dimension', null, 0, 2);

table_section_title(__("User Interface Options"));

check_row(__("Short Name and Name in List"), 'shortname_name_in_list', $_POST['shortname_name_in_list']);
check_row(__("Open Print Dialog Direct on Reports"), 'print_dialog_direct', null);
check_row(__("Search Item List"), 'no_item_list', null);
check_row(__("Search Customer List"), 'no_customer_list', null);
check_row(__("Search Supplier List"), 'no_supplier_list', null);
text_row_ex(__("Login Timeout:"), 'login_tout', 10, 10, '', null, null, __('seconds'));
text_row_ex(__("Max day range in documents"), 'max_days_in_docs', 10, 10, '', null, null, __('days.'));

end_outer_table(1);

hidden('coy_logo', $_POST['coy_logo']);
submit_center('update', __("Update"), true, '',  'default');
