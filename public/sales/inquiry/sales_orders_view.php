...
if ($trans_type == ST_SALESORDER)
{
	if (isset($_GET['OutstandingOnly']) && ($_GET['OutstandingOnly'] == true))
	{
		...
		$_SESSION['page_title'] = __($GLOBALS['help_context'] = "Search Outstanding Sales Orders");
	}
	elseif (isset($_GET['InvoiceTemplates']) && ($_GET['InvoiceTemplates'] == true))
	{
		$_POST['order_view_mode'] = 'InvoiceTemplates';
		$_SESSION['page_title'] = __($GLOBALS['help_context'] = "Search Template for Invoicing");
	}
	elseif (isset($_GET['DeliveryTemplates']) && ($_GET['DeliveryTemplates'] == true))
	{
		$_POST['order_view_mode'] = 'DeliveryTemplates';
		$_SESSION['page_title'] = __($GLOBALS['help_context'] = "Select Template for Delivery");
	}
	elseif (isset($_GET['PrepaidOrders']) && ($_GET['PrepaidOrders'] == true))
	{
		$_POST['order_view_mode'] = 'PrepaidOrders';
		$_SESSION['page_title'] = __($GLOBALS['help_context'] = "Invoicing Prepayment Orders");
	}
	elseif (!isset($_POST['order_view_mode']))
	{
		$_POST['order_view_mode'] = false;
		$_SESSION['page_title'] = __($GLOBALS['help_context'] = "Search All Sales Orders");
	}
}
else
{
	$_POST['order_view_mode'] = "Quotations";
	$_SESSION['page_title'] = __($GLOBALS['help_context'] = "Search All Sales Quotations");
}
...
{
	global $trans_type;
	return print_document_link($row['order_no'], __("Print"), true, $trans_type, ICON_PRINT);
}
...
function dispatch_link($row)
{
	...
	if ($trans_type == ST_SALESORDER)
	{
		if ($row['TotDelivered'] < $row['TotQuantity'] && !$page_nested)
			return pager_link( __("Dispatch"),
				"/sales/customer_delivery.php?OrderNumber=" .$row['order_no'], ICON_DOC);
		else
			return '';
	}		
	else
  		return pager_link( __("Sales Order"),
...
// save also in hidden field for testing during 'Update'
 ...
 return checkbox(null, $name, $value, true,
 	__('Set this order as a template for direct deliveries/invoices'))
...
function invoice_prep_link($row)
{
	...
	return 
		...
		pager_link($row['ord_payments']  ? __("Prepayment Invoice") : __("Final Invoice"),
...
start_row();
ref_cells(__("#:"), 'OrderNumber', '',null, '', true);
ref_cells(__("Ref"), 'OrderReference', '',null, '', true);

if ($show_dates)
    yesno_list_cells('', 'by_delivery', null, ($trans_type==ST_SALESORDER ? __("Delivery date") : __("Valid until")).':',
         ($trans_type==ST_SALESORDER ? __("Order date") : __("Quotation date")).':');

if ($show_dates)
{
  	date_cells(__("from:"), 'OrdersAfterDate', '', null, -user_transaction_days());
  	date_cells(__("to:"), 'OrdersToDate', '', null, 1);
}

locations_list_cells(__("Location:"), 'StockLocation', null, true, true);
...
stock_items_list_cells(__("Item:"), 'SelectStockFromList', null, true, true);
...
if ($trans_type == ST_SALESORDER)
	$cols = array(
		__("Order #") => array('fun'=>'view_link', 'align'=>'right', 'ord' =>''),
		__("Ref") => array('type' => 'sorder.reference', 'ord' => '') ,
        __("Tracking No") => array('type' => 'sorder.tracking_no', 'ord' => '') ,
		__("Customer") => array('type' => 'debtor.name' , 'ord' => '') ,
		__("Branch"), 
		__("Cust Order Ref"),
		__("Order Date") => array('type' =>  'date', 'ord' => ''),
		__("Required By") =>array('type'=>'date', 'ord'=>''),
		__("Delivery To"), 
		__("Order Total") => array('type'=>'amount', 'ord'=>''),
		'Type' => 'skip',
		__("Currency") => array('align'=>'center')
	);
else
	$cols = array(
		__("Quote #") => array('fun'=>'view_link', 'align'=>'right', 'ord' => ''),
		__("Ref"),
        __("Tracking No"),
		__("Customer"),
		__("Branch"), 
		__("Cust Order Ref"),
		__("Quote Date") => 'date',
		__("Valid until") =>array('type'=>'date', 'ord'=>''),
		__("Delivery To"), 
		__("Quote Total") => array('type'=>'amount', 'ord'=>''),
		'Type' => 'skip',
		__("Currency") => array('align'=>'center')
	);
if ($_POST['order_view_mode'] == 'OutstandingOnly') {
	...
	array_substitute($cols, 5, 1, __("Description"));
	...
	array_append($cols, array(
		__("New Payments") => array('insert'=>true, 'fun'=>'unallocated_prepayments'),
	 ...
	 array_append($cols,array(
			__("Tmpl") => array('insert'=>true, 'fun'=>'tmpl_checkbox'),
					array('insert'=>true, 'fun'=>'edit_link'),
					array('insert'=>true, 'fun'=>'dispatch_link'),
					array('insert'=>true, 'fun'=>'prt_link')));
};
...
submit_center('Update', __("Update"), true, '', null);
